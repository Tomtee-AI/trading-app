...
CACHE_FILE = Path("market_cache_dual_horizon.pkl")
...
def sign_score(condition_up: bool, condition_down: bool, magnitude: int = 1) -> int:
    if condition_up:
        return magnitude
    if condition_down:
        return -magnitude
...
def score_to_confidence(total_score: float, component_count: int, penalties: float = 0.0) -> float:
    normalized = min(1.0, abs(total_score) / max(component_count, 1))
    confidence = 0.50 + 0.40 * normalized - penalties
...
def fetch_market_snapshot() -> Dict[str, object]:
    ...
    for symbol in ALL_YF_SYMBOLS:
        try:
            ...
            data[symbol] = build_symbol_features(symbol, df, include_sma_100_200=(symbol == QQQ_SYMBOL or symbol == FUTURES_SYMBOL))
...
# -------------------- FACTOR SCORING --------------------
def generic_range_score(features: Dict[str, object], use_long_window: bool = False) -> Tuple[int, Dict[str, object]]:
    score = 0
    components = {}
    ...
    if prev_week_state == "ABOVE_PREVIOUS_WEEK_HIGH":
        ...
        score -= 1
    components["vs_previous_week"] = prev_week_state

    current_week_pos = features.get("current_week_position")
    if current_week_pos is not None:
        if current_week_pos >= 0.75:
            score += 1
        elif current_week_pos <= 0.25:
            score -= 1
    components["current_week_zone"] = features.get("current_week_zone")
    ...
    if daily_pos is not None:
        if daily_pos >= 0.67:
            ...
            score -= 1
    components["daily_zone"] = features.get("daily_zone")
    ...
    if vs_sma_21 == "ABOVE":
        ...
        score -= 1
    components["vs_sma_21"] = vs_sma_21
    ...
    if use_long_window:
        ...
        if range_63_pos is not None:
            if range_63_pos >= 0.60:
                ...
                score -= 1
        components["range_63_zone"] = features.get("range_63_zone")

    return score, components


def score_qqq_short(features: Dict[str, object]) -> Tuple[int, Dict[str, object], float]:
    score, components = generic_range_score(features, use_long_window=False)
    penalties = 0.0

    expansion_risk = False
    near_100 = features.get("vs_sma_100") == "NEAR"
    near_200 = features.get("vs_sma_200") == "NEAR"
    if near_100 or near_200:
        expansion_risk = True
        penalties += 0.05

    components["vs_sma_100"] = features.get("vs_sma_100")
    components["vs_sma_200"] = features.get("vs_sma_200")
    components["expansion_risk_near_big_ma"] = expansion_risk
    return score, components, penalties


def score_qqq_long(features: Dict[str, object]) -> Tuple[int, Dict[str, object], float]:
    score, components = generic_range_score(features, use_long_window=True)
    penalties = 0.0
    ...
    if sma100 is not None and sma200 is not None:
        if sma100 > sma200:
            score += 1
            components["sma_100_vs_200"] = "ABOVE"
        elif sma100 < sma200:
            score -= 1
            components["sma_100_vs_200"] = "BELOW"
        else:
            components["sma_100_vs_200"] = "FLAT"
    else:
        components["sma_100_vs_200"] = "UNKNOWN"
...
def score_macro_price(macro: Dict[str, object]) -> Tuple[int, Dict[str, str]]:
    rates = macro["rates"]
    fx = macro["fx"]
    liq = macro["liquidity"]
    ...
    if spread is None or change_5d is None:
        ...
        score += 1
        details["curve_signal"] = "MILD_RISK_ON"
    elif spread < 0:
        score -= 1
        details["curve_signal"] = "MILD_RISK_OFF"
    else:
        details["curve_signal"] = "NEUTRAL"
    ...
    rrp_5d = liq.get("change_5d_bil")
    rrp_20d = liq.get("change_20d_bil")
    if rrp_5d is None or rrp_20d is None:
        details["rrp_liquidity_signal"] = "NEUTRAL"
    elif rrp_5d < 0 and rrp_20d < 0:
        score += 1
...
def score_futures_context(features: Optional[Dict[str, object]], long_term: bool = False) -> Tuple[int, Dict[str, object]]:
    if not features:
        return 0, {"futures_context": "UNAVAILABLE"}

    score, base = generic_range_score(features, use_long_window=long_term)
    return score, {
        "futures_context": "AVAILABLE",
...
def score_vol_curve_short(curve: Dict[str, object]) -> Tuple[int, Dict[str, object]]:
    score = curve["backwardation_pairs"] - curve["contango_pairs"]
    score += sign_score(curve.get("breadth_above_sma21", 0) >= 4, curve.get("breadth_below_sma21", 0) >= 4, 1)

    short_state = "MIXED"
    if curve["backwardation_pairs"] >= 4:
        short_state = "BULLISH_VOL"
    elif curve["contango_pairs"] >= 4:
        short_state = "BEARISH_VOL"

    details = {
        "curve_short_state": short_state,
...
def score_vol_curve_long(curve: Dict[str, object]) -> Tuple[int, Dict[str, object]]:
    ...
    if mid_slope is not None:
        score += -1 if mid_slope > 0 else 1 if mid_slope < 0 else 0
    if back_slope is not None:
        score += -1 if back_slope > 0 else 1 if back_slope < 0 else 0
    if far_slope is not None:
        score += -1 if far_slope > 0 else 1 if far_slope < 0 else 0

    score += sign_score(curve.get("breadth_above_sma21", 0) >= 5, curve.get("breadth_below_sma21", 0) >= 5, 1)

    long_state = "MIXED"
    if score <= -2:
        long_state = "BEARISH_VOL"
    elif score >= 2:
        long_state = "BULLISH_VOL"

    details = {
        "curve_long_state": long_state,
...
def score_stress_asset(features: Dict[str, object], long_term: bool = False) -> Tuple[int, Dict[str, object]]:
    score, details = generic_range_score(features, use_long_window=long_term)
    if long_term:
        if features.get("range_126_position") is not None:
            if features["range_126_position"] >= 0.60:
                score += 1
            elif features["range_126_position"] <= 0.40:
                score -= 1
        details["range_126_zone"] = features.get("range_126_zone")
    return score, details


def score_horizon_bias(score: float, bullish_threshold: int = 2, bearish_threshold: int = -2) -> str:
...
def build_short_term_block(market: Dict[str, object], macro: Dict[str, object]) -> Dict[str, object]:
    ...
    if not qqq:
        raise ValueError("QQQ data is required for short-term scoring")

    qqq_score, qqq_details, qqq_penalty = score_qqq_short(qqq)
    macro_score, macro_details = score_macro_price(macro)
    ...
    vvix_score, vvix_details = score_stress_asset(vvix, long_term=False) if vvix else (0, {"status": "UNAVAILABLE"})
    move_score, move_details = score_stress_asset(move, long_term=False) if move else (0, {"status": "UNAVAILABLE"})

    price_score = qqq_score + macro_score + futures_score
    vol_score = curve_score + vvix_score + move_score

    price_bias = score_horizon_bias(price_score)
    vol_bias = score_horizon_bias(vol_score)
    regime = (
        "BULLISH_PRICE_BEARISH_VOL" if price_bias == "BULLISH" and vol_bias == "BEARISH"
        else "BEARISH_PRICE_BULLISH_VOL" if price_bias == "BEARISH" and vol_bias == "BULLISH"
        else "NEUTRAL" if price_bias == "NEUTRAL" and vol_bias == "NEUTRAL"
        else "MIXED"
    )

    component_count = 3 + 3 + 2  # qqq + macro + vol complex, approximate for confidence scaling
    penalties = qqq_penalty
    confidence = score_to_confidence(price_score + vol_score, component_count, penalties)
...
def build_long_term_block(market: Dict[str, object], macro: Dict[str, object]) -> Dict[str, object]:
    symbols = market["symbols"]
    qqq = symbols.get(QQQ_SYMBOL)
    vvix = symbols.get(VVIX_SYMBOL)
    move = symbols.get(MOVE_SYMBOL)
    ...
    if not qqq:
        raise ValueError("QQQ data is required for long-term scoring")
    ...
    vvix_score, vvix_details = score_stress_asset(vvix, long_term=True) if vvix else (0, {"status": "UNAVAILABLE"})
    move_score, move_details = score_stress_asset(move, long_term=True) if move else (0, {"status": "UNAVAILABLE"})
...
def build_market_decision() -> Dict[str, object]:
    cached = load_cache()
    ...
    short_term = build_short_term_block(market_snapshot, macro_snapshot)
    long_term = build_long_term_block(market_snapshot, macro_snapshot)
...
if __name__ == "__main__":
    try:
        ...
        report_path = REPORTS_DIR / f"market_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
