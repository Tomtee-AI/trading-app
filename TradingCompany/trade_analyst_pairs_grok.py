#!/usr/bin/env python3
"""
trade_analyst_pairs.py (refactored)

Standalone Trade Analyst for metals relative-value trading (futures signal) + equity/options execution layer.

Added in this refactor
----------------------
1. Strict JSON schema validation remains intact (bumped to schema_version 1.2.0)
2. When a futures pair candidate is identified (e.g. GC expensive vs SI cheap):
   - Maps to equity/ETF universe per metal (user-specified)
   - Ranks stocks/ETFs within each metal group by % deviation from SMA63
   - Selects most expensive peer (sell) in expensive-metal group and most undervalued peer (buy) in cheap-metal group
   - Adds concrete stock long/short recommendation + simple options execution idea
3. New "equity_drilldown" attached to top_candidate
4. New top-level "equity_analysis" in components
5. All original futures logic, cache, validation, and NO_TRADE paths unchanged

Symbols used for actual trades:
- Gold:     GDX, GDXJ, GLD, B
...
# -------------------- EQUITY PROXIES FOR TRADE EXECUTION --------------------
EQUITY_GROUPS = {
    "GC=F": ["GDX", "GDXJ", "GLD", "B"],      # Gold
    "SI=F": ["AG", "HL", "PAAS", "SLV"],      # Silver
    "HG=F": ["BHP", "FCX", "RIO"],            # Copper
    "PL=F": ["PLTM"],                         # Platinum
    "PA=F": [],                               # Palladium (none provided)
}
...
EQUITY_LABELS = {
    ...
    "RIO": "Rio Tinto Group",
    "PLTM": "Platinum Group Metals ETF",
}
...
SYMBOL_FEATURE_SCHEMA = {
    ...
    "required": [
        "symbol", "alias", "label", "as_of", "rows", "previous_close",
        "effective_price", "price_source", "source_timestamp_utc",
        "daily_high", "daily_low", "sma_21", "sma_63", "vs_sma_21", "vs_sma_63",
    ],
    "properties": {
        "symbol": {"type": "string"},
        "alias": {"type": "string"},
...
PAIR_EVAL_SCHEMA = {
    ...
    "additionalProperties": False,
    "required": [
        "pair_id", "pair_label", "symbol_a", "symbol_b", "alias_a", "alias_b",
        "label_a", "label_b", "status", "reason_code", "correlation",
        "hedge_ratio_beta", "last_close_zscore", "current_zscore",
        "expensive_symbol", "cheap_symbol", "recommendation", "confidence",
    ...
    "properties": {
        ...
        "inputs": {
            ...
            "additionalProperties": False,
            "required": [
                "effective_price_a", "effective_price_b", "price_source_a",
                "price_source_b", "rows_a", "rows_b", "usable_rows",
            ],
            "properties": {
        ...
        # NEW: equity/options execution layer (optional on candidates)
        "equity_drilldown": {"type": ["object", "null"], "additionalProperties": True},
    },
}
    ...
    "properties": {
        ...
        "components": {
            ...
            "properties": {
                ...
                "equity_analysis": {"type": ["object", "null"], "additionalProperties": True},
            },
        },
        "summary": {"type": "string"},
    },
...
def build_instrument_features(symbol: str, df: pd.DataFrame, config: RuntimeConfig, custom_label: Optional[str] = None) -> Dict[str, object]:
    close_series = df["Close"].dropna()
    ...
    return {
        ...
        "label": custom_label or METALS_SYMBOLS.get(symbol, symbol),
...
def fetch_market_snapshot(config: RuntimeConfig) -> Dict[str, object]:
    ...
    for symbol in METALS_SYMBOLS:
        try:
            ...
            symbol_features[symbol] = build_instrument_features(symbol, df, config, METALS_SYMBOLS[symbol])
...
def build_equity_feature(symbol: str, df: pd.DataFrame, config: RuntimeConfig) -> Dict[str, object]:
    ...
    return build_instrument_features(symbol, df, config, EQUITY_LABELS.get(symbol, symbol))
...
def fetch_equity_snapshot(equity_list: List[str], config: RuntimeConfig) -> Dict[str, object]:
    ...
    return {
        "fetched_at": now_iso(),
        "frames": equity_frames,
        "symbols": equity_features,
        "errors": errors,
    }
...
def build_summary(status: str, top_candidate: Optional[Dict[str, object]], no_trade: Optional[Dict[str, object]]) -> str:
    if status == "OK" and top_candidate:
        ...
        if top_candidate.get("equity_drilldown") and top_candidate["equity_drilldown"].get("suggested_sell_stock"):
            eq = top_candidate["equity_drilldown"]
            base += f" Equity execution → SELL {eq['suggested_sell_stock']['symbol']} / BUY {eq['suggested_buy_stock']['symbol']}."
        return base
...
# -------------------- DECISION BUILDER (with equity/options) --------------------
def build_trade_decision(config: RuntimeConfig) -> Dict[str, object]:
    ...
    # === EQUITY / OPTIONS EXECUTION LAYER ===
    equity_analysis = None
    if top_candidate and top_candidate.get("status") == "CANDIDATE":
        ...
        if all_eq:
            try:
                ...
                equity_analysis = {
                    ...
                    "stock_recommendation": f"SELL {sug_sell['symbol']} / BUY {sug_buy['symbol']}" if sug_sell and sug_buy else None,
                    "options_idea": (
                        f"Sell calls (or short stock) on {sug_sell['symbol']} (expensive) / "
                        f"Buy calls on {sug_buy['symbol']} (cheap). Consider vertical spreads for defined risk."
                        if sug_sell and sug_buy else None
                    ),
                }
                top_candidate["equity_drilldown"] = equity_analysis
            except Exception as e:
                equity_analysis = {"error": str(e)}

    metals_list = [market_snapshot["symbols"][symbol] for symbol in sorted(market_snapshot["symbols"].keys())]
    price_sources = [{"symbol": f["symbol"], "alias": f["alias"], "price_source": f["price_source"]} for f in metals_list]
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Metals relative-value Trade Analyst (with equity/options execution)")
    parser.add_argument(
        "--pairs",
        type=str,
