#!/usr/bin/env python3
"""
trade_analyst_pairs_refactored.py

Standalone Trade Analyst for metals relative-value trading with stock and options overlays.

What this version adds
----------------------
- Keeps the metals-futures pair engine from the prior version
- After a metals pair candidate passes filters, maps each commodity leg to an equity/ETF peer group
- Scores the peers to determine which stock/ETF is expensive or cheap relative to its own peer basket
- Builds actual trade expressions using:
    - stock leg ideas
    - options leg ideas (template + best-effort expiration lookup)
- Preserves strict JSON schema validation and NO_TRADE blocks

Notes
-----
- This script is analysis-only. It does not route or place orders.
- Options ideas are templates with best-effort expiration lookup, not broker-ready order tickets.
- The gold peer list includes ticker "B" exactly as provided by the user.
"""
...
CACHE_FILE = Path("trade_analyst_pairs_cache.pkl")
...
DEFAULT_ALLOWED_PAIR_TEXT = "GC/SI,PL/PA,GC/PA"

COMMODITY_TO_EQUITY_PEERS = {
    "gold": ["GDX", "GDXJ", "GLD", "B"],
    "silver": ["AG", "HL", "PAAS", "SLV"],
    "copper": ["BHP", "FCX", "RIO"],
    "platinum": ["PLTM"],
}
...
class RuntimeConfig:
    ...
    peer_beta_window: int = 126
    peer_zscore_window: int = 63
    min_corr: float = 0.45
...
STOCK_PEER_SCHEMA = {
    ...
    "properties": {
        ...
        "symbol": {"type": "string"},
        "status": {"type": "string", "enum": ["CANDIDATE", "DISQUALIFIED", "ERROR"]},
        "reason_code": {"type": "string", "enum": STOCK_REASON_CODES},
        "effective_price": {"type": ["number", "null"]},
        "price_source": {"type": ["string", "null"]},
        "peer_zscore": {"type": ["number", "null"]},
        "relative_rank": {"type": ["string", "null"], "enum": ["EXPENSIVE", "CHEAP", None]},
        "rows": {"type": "integer", "minimum": 0},
        "options": {
            "type": "object",
            "additionalProperties": False,
            "required": ["options_available", "selected_expiration", "preferred_structure", "template"],
            "properties": {
                "options_available": {"type": "boolean"},
                "selected_expiration": {"type": ["string", "null"]},
                "preferred_structure": {"type": ["string", "null"]},
                "template": {
                    "type": ["object", "null"],
                    "additionalProperties": False,
                    "required": ["bias", "dte_target_range", "long_leg_delta_target", "short_leg_delta_target"],
                    "properties": {
                        "bias": {"type": "string", "enum": ["BULLISH", "BEARISH"]},
                        "dte_target_range": {"type": "string"},
                        "long_leg_delta_target": {"type": "number"},
                        "short_leg_delta_target": {"type": "number"},
                    },
                },
            },
        },
    ...
    "properties": {
        ...
        "stock_trade_plan": {
            ...
            "items": {
                "type": "object",
                "additionalProperties": False,
                "required": ["symbol", "commodity_group", "side", "action", "rationale"],
                "properties": {
                    "symbol": {"type": "string"},
                    "commodity_group": {"type": "string"},
                    "side": {"type": "string", "enum": ["LONG", "SHORT"]},
                    "action": {"type": "string", "enum": ["BUY_STOCK", "SELL_STOCK"]},
        ...
        "options_trade_plan": {
            ...
            "items": {
                "type": "object",
                "additionalProperties": False,
                "required": ["symbol", "commodity_group", "bias", "structure", "expiration", "template", "rationale"],
                "properties": {
                    "symbol": {"type": "string"},
                    "commodity_group": {"type": "string"},
                    "bias": {"type": "string", "enum": ["BULLISH", "BEARISH"]},
                    "structure": {"type": "string", "enum": ["CALL_DEBIT_SPREAD", "PUT_DEBIT_SPREAD", "NO_OPTIONS_FOUND"]},
                    "expiration": {"type": ["string", "null"]},
                    "template": {
                        "type": ["object", "null"],
                        "additionalProperties": False,
                        "required": ["dte_target_range", "long_leg_delta_target", "short_leg_delta_target"],
                        "properties": {
                            "dte_target_range": {"type": "string"},
                            "long_leg_delta_target": {"type": "number"},
                            "short_leg_delta_target": {"type": "number"},
...
NO_TRADE_SCHEMA = {
    "type": "object",
    "additionalProperties": False,
    "required": ["reason_codes", "reason_counts", "next_action", "summary"],
    "properties": {
        "reason_codes": {
            "type": "array",
            "minItems": 1,
        ...
        "reason_counts": {
            ...
            "properties": {
                ...
                "actual_trade_ready_count": {"type": "integer", "minimum": 0},
    ...
    "properties": {
        ...
        "decision": {
            ...
            "properties": {
                ...
                "candidate_count": {"type": "integer", "minimum": 0},
                "actual_trade_count": {"type": "integer", "minimum": 0},
                "top_candidate": {"anyOf": [{"type": "null"}, PAIR_EVAL_SCHEMA]},
                "top_actual_trade": {"anyOf": [{"type": "null"}, ACTUAL_TRADE_SCHEMA]},
                "allowed_pairs": {"type": "array", "items": {"type": "string"}, "uniqueItems": True},
                "thresholds": {
                    "type": "object",
                    "additionalProperties": False,
                    "required": [
                        ...
                        "beta_window_days",
                        "zscore_window_days",
                        "peer_beta_window_days",
                        "peer_zscore_window_days",
                    ],
                    "properties": {
                        ...
                        "peer_beta_window_days": {"type": "integer", "minimum": 1},
                        "peer_zscore_window_days": {"type": "integer", "minimum": 1},
                    },
                },
                "no_trade": {"anyOf": [{"type": "null"}, NO_TRADE_SCHEMA]},
            },
        },
        "components": {
            "type": "object",
            "additionalProperties": False,
            "required": ["metals", "pair_analytics", "trade_candidates", "equity_peer_analytics", "actual_trades"],
            "properties": {
                "metals": {"type": "array", "items": SYMBOL_FEATURE_SCHEMA},
                "pair_analytics": {"type": "array", "items": PAIR_EVAL_SCHEMA},
                "trade_candidates": {"type": "array", "items": PAIR_EVAL_SCHEMA},
                "equity_peer_analytics": {"type": "array", "items": STOCK_PEER_SCHEMA},
                "actual_trades": {"type": "array", "items": ACTUAL_TRADE_SCHEMA},
            },
        },
        "summary": {"type": "string"},
    },
...
def parse_pair_allowlist(pair_text: str) -> Tuple[PairSpec, ...]:
    ...
    for raw in raw_items:
        ...
        parsed.append(PairSpec(symbol_a=symbol_a, symbol_b=symbol_b, alias_a=alias_a, alias_b=alias_b))
...
def build_symbol_features(symbol: str, df: pd.DataFrame, config: RuntimeConfig) -> Dict[str, object]:
    ...
    label = METALS_SYMBOLS.get(symbol, symbol.lower())
...
def fetch_market_snapshot(config: RuntimeConfig) -> Dict[str, object]:
    ...
    equity_symbols = sorted({sym for peers in COMMODITY_TO_EQUITY_PEERS.values() for sym in peers})
    for symbol in equity_symbols:
        try:
            df = fetch_history(symbol, period=config.daily_period, interval=config.daily_interval)
            equity_frames[symbol] = df
            equity_features[symbol] = build_symbol_features(symbol, df, config)
        except Exception as exc:
            equity_errors[symbol] = str(exc)
...
def choose_option_expiration(symbol: str, min_dte: int, max_dte: int) -> Tuple[bool, Optional[str]]:
    ...
    for exp in expirations:
        ...
        if dte < min_dte or dte > max_dte:
            continue
        dist = abs(dte - ((min_dte + max_dte) // 2))
        if dist < best_dist:
            best_dist = dist
            best_exp = exp
...
def build_options_template(bias: str, config: RuntimeConfig) -> Dict[str, object]:
    if bias == "BULLISH":
        return {
            "bias": "BULLISH",
...
def empty_peer_result(group: str, symbol: str, reason_code: str, status: str = "DISQUALIFIED") -> Dict[str, object]:
    return {
        "group": group,
        "symbol": symbol,
        "status": status,
...
def score_peer_group(group: str, market_snapshot: Dict[str, object], config: RuntimeConfig) -> List[Dict[str, object]]:
    peers = COMMODITY_TO_EQUITY_PEERS.get(group, [])
    if not peers:
        return []

    equity_features = market_snapshot["equity_features"]
    equity_frames = market_snapshot["equity_frames"]
    available = [sym for sym in peers if sym in equity_features and sym in equity_frames and not equity_frames[sym].empty]

    if not available:
        return [
            {
                **empty_peer_result(group, sym, "MISSING_SYMBOL_DATA"),
                "rationale": f"No usable data for {group} peer {sym}.",
            }
            for sym in peers
        ]
    ...
    if len(available) == 1:
        ...
        result = empty_peer_result(group, symbol, "SINGLE_NAME_GROUP")
        result.update({
            "status": "CANDIDATE",
            "effective_price": equity_features[symbol].get("effective_price"),
            "price_source": equity_features[symbol].get("price_source"),
            "rows": int(equity_features[symbol].get("rows", 0)),
            "options": {
            ...
            "rationale": f"{group} has only one mapped tradable peer; relative rank versus peers cannot be computed.",
        })
        return [result]
    ...
    min_needed = max(config.peer_beta_window, config.peer_zscore_window) + 5
    if len(close_df) < min_needed:
        return [
            {
                **empty_peer_result(group, sym, "INSUFFICIENT_HISTORY"),
                "effective_price": equity_features[sym].get("effective_price") if sym in equity_features else None,
                "price_source": equity_features[sym].get("price_source") if sym in equity_features else None,
                "rows": int(row_map.get(sym, 0)),
                "rationale": f"Peer group {group} has only {len(close_df)} overlapping rows; {min_needed} required.",
            }
            for sym in peers
        ]
    ...
    for sym in available:
        ...
        x = sample["basket"].tail(config.peer_beta_window).values
        y = sample[sym].tail(config.peer_beta_window).values
        var_x = np.var(x, ddof=1)
        if not np.isfinite(var_x) or var_x <= 0:
            zscores[sym] = None
            continue
        ...
        z_window = spread.tail(config.peer_zscore_window)
        sigma = safe_float(z_window.std(ddof=1))
        if sigma is None or sigma <= 1e-10:
            zscores[sym] = None
            continue

        eff = equity_features[sym].get("effective_price")
        if eff in (None, 0):
            zscores[sym] = None
            continue
        ...
        z = safe_float((current_spread - safe_float(z_window.mean(), 0.0)) / sigma)
        zscores[sym] = z
    ...
    for sym in peers:
        if sym not in equity_features:
            result = empty_peer_result(group, sym, "MISSING_SYMBOL_DATA")
            result["rationale"] = f"Peer {sym} missing from equity data snapshot."
            results.append(result)
            continue

        feature = equity_features[sym]
        options_available, expiration = choose_option_expiration(sym, config.option_target_min_dte, config.option_target_max_dte)
        z = zscores.get(sym)
        result = empty_peer_result(group, sym, "PASSED", status="CANDIDATE")
        result.update({
            ...
            "rationale": f"{sym} ranked within {group} peer basket using hedge-adjusted peer spread z-score.",
        })

        if z is None:
            result["status"] = "DISQUALIFIED"
            result["reason_code"] = "INTERNAL_ERROR"
            result["rationale"] = f"{sym} peer z-score could not be computed."
        elif sym == expensive:
            ...
            result["rationale"] = f"{sym} is in the middle of its {group} peer ranking and not the best actual trade vehicle."
...
def build_equity_peer_snapshot(market_snapshot: Dict[str, object], config: RuntimeConfig) -> Dict[str, object]:
    analytics: List[Dict[str, object]] = []
    for group in COMMODITY_TO_EQUITY_PEERS:
        analytics.extend(score_peer_group(group, market_snapshot, config))
    return {"equity_peer_analytics": analytics}


def choose_actual_trade_leg(group: str, side: str, equity_peer_analytics: List[Dict[str, object]]) -> Optional[Dict[str, object]]:
    target_rank = "CHEAP" if side == "LONG" else "EXPENSIVE"
    candidates = [
        row for row in equity_peer_analytics
        if row["group"] == group and row["status"] == "CANDIDATE" and row["relative_rank"] == target_rank
    ]
    if not candidates:
        return None
...
def build_actual_trade_for_candidate(candidate: Dict[str, object], equity_peer_analytics: List[Dict[str, object]]) -> Dict[str, object]:
    expensive_group = METALS_SYMBOLS.get(candidate["expensive_symbol"])
    cheap_group = METALS_SYMBOLS.get(candidate["cheap_symbol"])

    short_leg = choose_actual_trade_leg(expensive_group, "SHORT", equity_peer_analytics) if expensive_group else None
    long_leg = choose_actual_trade_leg(cheap_group, "LONG", equity_peer_analytics) if cheap_group else None

    stock_trade_plan: List[Dict[str, object]] = []
    options_trade_plan: List[Dict[str, object]] = []

    if short_leg:
        stock_trade_plan.append({
            "symbol": short_leg["symbol"],
            "commodity_group": expensive_group,
            "side": "SHORT",
            "action": "SELL_STOCK",
            "rationale": f"{short_leg['symbol']} is the most expensive equity vehicle in the {expensive_group} peer group.",
        })
        options_trade_plan.append({
            ...
            "template": {
                "dte_target_range": short_leg["options"]["template"]["dte_target_range"],
                "long_leg_delta_target": short_leg["options"]["template"]["long_leg_delta_target"],
                "short_leg_delta_target": short_leg["options"]["template"]["short_leg_delta_target"],
            } if short_leg["options"]["template"] else None,
    ...
    if long_leg:
        stock_trade_plan.append({
            ...
            "commodity_group": cheap_group,
            "side": "LONG",
            "action": "BUY_STOCK",
            "rationale": f"{long_leg['symbol']} is the cheapest equity vehicle in the {cheap_group} peer group.",
        })
        options_trade_plan.append({
            "symbol": long_leg["symbol"],
            "commodity_group": cheap_group,
            ...
            "template": {
                "dte_target_range": long_leg["options"]["template"]["dte_target_range"],
                "long_leg_delta_target": long_leg["options"]["template"]["long_leg_delta_target"],
                "short_leg_delta_target": long_leg["options"]["template"]["short_leg_delta_target"],
            } if long_leg["options"]["template"] else None,
    ...
    rationale = (
        f"Translate {candidate['pair_label']} metals signal into equity vehicles by shorting the richest {expensive_group} proxy and "
        f"buying the cheapest {cheap_group} proxy."
        if short_leg and long_leg else
        "Could not fully map both commodity legs to equity vehicles."
    )
...
def build_actual_trade_snapshot(pair_snapshot: Dict[str, object], equity_peer_snapshot: Dict[str, object]) -> Dict[str, object]:
    actual_trades = []
    for candidate in pair_snapshot["trade_candidates"]:
        actual_trades.append(build_actual_trade_for_candidate(candidate, equity_peer_snapshot["equity_peer_analytics"]))

    actual_trades.sort(key=lambda x: {"READY": 0, "PARTIAL": 1, "UNAVAILABLE": 2}[x["status"]])
    return {"actual_trades": actual_trades}


def build_no_trade_block(pair_snapshot: Dict[str, object], market_snapshot: Dict[str, object], actual_trades: List[Dict[str, object]]) -> Dict[str, object]:
    counts = dict(pair_snapshot["counts"])
    counts["actual_trade_ready_count"] = sum(1 for row in actual_trades if row["status"] == "READY")
    ...
    if pair_snapshot["counts"]["candidate_count"] > 0 and counts["actual_trade_ready_count"] == 0:
        reason_codes.append("NO_STOCK_PEER_CANDIDATES")

    if market_snapshot.get("errors") or market_snapshot.get("equity_errors"):
    ...
    elif "NO_ALLOWED_PAIRS_CONFIGURED" in reason_codes or "NO_STOCK_PEER_CANDIDATES" in reason_codes:
        next_action = "WIDEN_UNIVERSE"

    summary = (
        f"No trade. Allowed pairs={counts['allowed_pair_count']}, pair candidates={pair_snapshot['counts']['candidate_count']}, "
        f"actual_trade_ready={counts['actual_trade_ready_count']}. Reason codes: {', '.join(reason_codes)}."
    )
...
def build_summary(status: str, top_candidate: Optional[Dict[str, object]], top_actual_trade: Optional[Dict[str, object]], no_trade: Optional[Dict[str, object]]) -> str:
    if status == "OK" and top_candidate and top_actual_trade:
        stock_text = ", ".join(f"{leg['action']} {leg['symbol']}" for leg in top_actual_trade["stock_trade_plan"]) or "no stock legs"
        return (
            f"Top metals RV opportunity is {top_candidate['recommendation']} in {top_candidate['pair_label']}. "
            f"Suggested implementation: {stock_text}."
        )
    if no_trade:
...
def build_trade_decision(config: RuntimeConfig) -> Dict[str, object]:
    ...
    equity_peer_snapshot = build_equity_peer_snapshot(market_snapshot, config)
    actual_trade_snapshot = build_actual_trade_snapshot(pair_snapshot, equity_peer_snapshot)

    trade_candidates = pair_snapshot["trade_candidates"]
    actual_trades = actual_trade_snapshot["actual_trades"]
    ready_actual_trades = [row for row in actual_trades if row["status"] == "READY"]

    top_candidate = trade_candidates[0] if trade_candidates else None
    top_actual_trade = ready_actual_trades[0] if ready_actual_trades else None

    status = "OK" if top_candidate and top_actual_trade else "NO_TRADE"
    no_trade = None if status == "OK" else build_no_trade_block(pair_snapshot, market_snapshot, actual_trades)

    metals_list = [market_snapshot["symbols"][symbol] for symbol in sorted(market_snapshot["symbols"].keys())]
    price_sources = [
        {
    ...
    decision = {
        ...
        "data_quality": {
            ...
            "equity_errors": sorted(list(market_snapshot.get("equity_errors", {}).keys())),
        },
        "decision": {
            "complex": "metals",
            ...
            "status": status,
            "candidate_count": len(trade_candidates),
            "actual_trade_count": len(ready_actual_trades),
            "top_candidate": top_candidate,
            "top_actual_trade": top_actual_trade,
            ...
            "thresholds": {
                ...
                "peer_beta_window_days": config.peer_beta_window,
                "peer_zscore_window_days": config.peer_zscore_window,
            },
            "no_trade": no_trade,
        },
        "components": {
            ...
            "trade_candidates": trade_candidates,
            "equity_peer_analytics": equity_peer_snapshot["equity_peer_analytics"],
            "actual_trades": actual_trades,
        },
        "summary": "",
    }

    decision["summary"] = build_summary(status, top_candidate, top_actual_trade, no_trade)
...
def add_llm_summary(decision: Dict[str, object]) -> Dict[str, object]:
    ...
    prompt = (
        "You are a concise trade analyst narrator. Return ONLY valid JSON with one key named summary. "
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Metals relative-value Trade Analyst with stock/options overlays")
    parser.add_argument(
        "--pairs",
        type=str,
    ...
    parser.add_argument("--peer-beta-window", type=int, default=126, help="Peer beta estimation window")
    parser.add_argument("--peer-zscore-window", type=int, default=63, help="Peer z-score window")
...
if __name__ == "__main__":
    try:
        ...
        config = RuntimeConfig(
            ...
            peer_beta_window=args.peer_beta_window,
            peer_zscore_window=args.peer_zscore_window,
        ...
        report_path = REPORTS_DIR / f"trade_report_pairs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
