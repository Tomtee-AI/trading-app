#!/usr/bin/env python3
"""
crack_spread_analyst_refactored_v3_2.py
...
class RuntimeConfig:
    ...
    min_abs_z: float = 1.0
    watchlist_min_abs_z: float = 0.75
    min_fit_score: float = 0.55
    watchlist_min_fit_score: float = 0.35
...
def build_candidate(
    ...
    refiner_bias = "BULLISH" if bullish_refiners else "BEARISH"
    margin_state = "EXPANDING" if bullish_refiners else "CONTRACTING"
    ...
    return {
        ...
        "direction": refiner_bias,
        "horizon": "SHORT_TERM",
        "structure_family": "crack_spread_options_overlay",
        "summary": (
            f"Refiner-margin signal is {refiner_bias}. Crack spread is {zscore:.2f}σ normalized | "
            f"margin={margin_state} | sigma={sigma_state} | EM={em_state} | proxy={selected_refiner} {refiner_side}"
        ),
        "confidence": round(min(0.95, fit_score + 0.15), 2),
        "fit_score": round(fit_score, 2),
        "thesis_tags": [
            "crack_spread", "refiner_margin", "sigma_reentry", "expected_move",
...
def build_watchlist_entry(
    ...
    return {
        "watchlist_id": f"CRACK_WL_{datetime.now().strftime('%Y%m%d%H%M')}",
        "strategy_type": "crack_spread",
        "direction": refiner_bias,
        "horizon": "SHORT_TERM",
        "status": "WATCHLIST",
        ...
        "summary": (
            f"Watchlist only: refiner-margin signal is {refiner_bias}. Crack spread is {zscore:.2f}σ normalized | "
            f"margin={margin_state} | sigma={sigma_state} | EM={em_state} | proxy={selected_refiner} {refiner_side}"
        ),
        "confidence": round(min(0.90, fit_score + 0.10), 2),
        "fit_score": round(fit_score, 2),
        "implementation": {
            "signal_basis": "REFINER_MARGIN",
            "refiner_bias": refiner_bias,
...
def evaluate_crack_with_options_context(config: RuntimeConfig, refiners_list: List[str]) -> Tuple[Optional[Dict[str, Any]], List[Dict[str, Any]], Dict[str, Any]]:
    ...
    try:
        ...
        if abs(zscore) < config.min_abs_z:
            if abs(zscore) >= config.watchlist_min_abs_z:
                watchlist_reason = f"WATCHLIST_ZSCORE_{abs(zscore):.4f}"
            else:
        ...
        if fit_score < config.min_fit_score:
            if watchlist_reason is None and fit_score >= config.watchlist_min_fit_score:
                watchlist_reason = f"WATCHLIST_FIT_SCORE_{fit_score:.4f}"
            elif watchlist_reason is None:
        ...
        if watchlist_reason is not None:
            diagnostics["watchlist_reason"] = watchlist_reason
            diagnostics["filter_reason"] = watchlist_reason
            wl = build_watchlist_entry(
                zscore=zscore,
                sigma_state=sigma_state,
                em_state=em_state,
                vol_up=vol_up,
            ...
            watchlist.append(wl)
            log(f"   👀 WATCHLIST: {watchlist_reason} (refiner={selected_refiner})", config.verbose)
            return None, watchlist, diagnostics
...
def build_crack_spread_decision(config: RuntimeConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    candidate, watchlist, diagnostics = evaluate_crack_with_options_context(config, refiners)
    ...
    if watchlist:
        log("👀 WATCHLIST SUMMARY", config.verbose)
        for w in watchlist:
            log(f"{w['watchlist_id']:40} | fit={w['fit_score']:.2f} | conf={w['confidence']:.2f} | {w['watchlist_reason']} | {w['summary'][:70]}...", config.verbose)
    log("=" * 90, config.verbose)
    ...
    output = {
        ...
        "summary": (
            f"Crack Spread Analyst identified {len(candidates)} refiner-margin opportunities and {len(watchlist)} watchlist setups "
    ...
    report = {
        ...
        "config": {
            ...
            "watchlist_min_abs_z": config.watchlist_min_abs_z,
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Refactored Crack Spread Analyst v3")
    ...
    parser.add_argument("--watchlist-min-abs-z", type=float, default=0.75)
    parser.add_argument("--watchlist-min-fit-score", type=float, default=0.35)
...
def main() -> int:
    ...
    config = RuntimeConfig(
        ...
        watchlist_min_abs_z=args.watchlist_min_abs_z,
    ...
    report_path = REPORTS_DIR / f"crack_spread_report_refactored_v3_2_{ts}.json"
